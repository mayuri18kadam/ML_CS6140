...
class myThread (threading.Thread):
    def __init__(self, threadID, counter, dataset, nfold, maxIter, learningRate, tol, lam):
       ...
       self.loss = []
    def run(self):
        acc_train, loss = evaluate_algorithmGradDecent(self.dataset, self.nfold, self.maxIter, self.learningRate,
                                    self.tol, self.lam, self.threadID, self.counter)
        threadLock.acquire()
        lossArrMin.append(np.min(loss))
        accArrAvg.append(np.average(acc_train))
        dispTol.append(self.tol)
        dispLearnRate.append(self.learningRate)
        displambda.append(self.lam)
        threadLock.release()

def calLossFunc(X, y, w, lam):
    loss = 0
    sigArr = []
    # val = (lam / 2)*(np.matmul(np.transpose(w), w)[0, 0])
    for rowIndex in range(len(X)):
        comp = np.dot(y[rowIndex], np.sum(np.dot(X[rowIndex], w)))
        sig = expit(comp)
        sigArr.append(sig)
        # loss = loss - log1p(sig) + val
        loss = loss - log1p(sig)
    return loss, sigArr
...
def getWeightsGradDecent(X, y, w, max_iter, learning_rate, tolerance, lam):
    ...
    for iterCount in range(max_iter):
        loss_new, sigArr = calLossFunc(X, y, w, lam)
        lossArr.append(loss_new)
        if abs(loss_new - loss) <= tolerance:
            return w, lossArr
        updated = []
        for rowIndex in range(len(X)):
            updated.append(sigArr[rowIndex])
        tempArr = np.sum((np.matmul(updated, np.transpose(np.matmul(np.transpose(X), y)))), axis=0)
        tempArr2 = np.matmul(np.transpose(X), y)
        for rowIndex in range(len(tempArr)):
            w[rowIndex] = w[rowIndex] + (learning_rate * (lam*w[rowIndex] + tempArr[rowIndex] - tempArr2[rowIndex]))
        loss = loss_new
    return w, lossArr
...
def getPredictedOutputAndWeightsGradDecent(X, y, max_iter, learning_rate, tolerance, lam):
    ...
    for rowIndex in range(len(X_final)):
        comp = np.dot(y[rowIndex], np.sum(np.dot(X_final[rowIndex], w)))
        sig = expit(comp)
        y_pred.append(sig)
