...
def calKernelFunc(x1, x2, power) :
    return math.pow(np.sum(np.dot(x1, x2)), power)

def getWeightsGradDecent(X, y, w, power):
    alpha = np.zeros((len(X), 1))
    for j in range(len(X)):
        temp = 0
        for k in range(len(X)) :
            temp = np.add(temp, (alpha[k] * y[k] * calKernelFunc(X[k], X[j], power)))
...
def getPredictedOutputAndWeightsGradDecent(X, y, power):
    ...
    y_pred = []
    w = getWeightsGradDecent(X_final, y, w, power)
...
def evaluate_algorithmGradDecent(dataset, n_folds, power):
    meanAcc_train = []
    meanRecall_train = []
    meanPre_train = []
    meanAcc_test = []
    ...
    for train_index, test_index in kf.split(dataset):
        ...
        y_pred_train, w_train = getPredictedOutputAndWeightsGradDecent(X_train, y_train, power)
...
def main():
    ...
    # dataset, n_folds, power
    acc_test = evaluate_algorithmGradDecent(twoSpirals.values, 10, 1)
